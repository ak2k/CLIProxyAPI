...
func TestClampClaudeRetryAfter(t *testing.T) {
	tests := []struct {
		name string
		in   *time.Duration
		want *time.Duration
		...
		{
			// Load-bearing: nil means "no header-derived cooldown", which is
			// what hands the decision back to the generic exponential backoff.
			// Mapping it to zero would pin every such retry at no delay.
			name: "nil passes through",
			in:   nil,
			want: nil,
		},
		{
			name: "zero is preserved as retry immediately",
			in:   durPtr(0),
			want: durPtr(0),
		},
		{
			name: "ordinary duration is untouched",
			in:   durPtr(time.Hour),
			want: durPtr(time.Hour),
		},
		{
			name: "far-future duration is capped",
			in:   durPtr(300 * 24 * time.Hour),
			want: durPtr(maxClaudeRetryAfter),
		},
		{
			name: "negative duration yields no cooldown",
			in:   durPtr(-time.Hour),
			want: nil,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := ClampClaudeRetryAfter(tc.in)
			if (tc.want == nil) != (got == nil) {
				t.Fatalf("want=%v got=%v", tc.want, got)
			}
