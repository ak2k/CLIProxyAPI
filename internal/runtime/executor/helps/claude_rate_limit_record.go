...
//
// The name needs its own bound: it is retained as the raw map's key and, via
// the suffix, as a window slug, so capping only the value leaves a capture
// sized by the header name instead. An over-long name is skipped rather than
// truncated, since truncating would fold two distinct names onto one key.
const (
	maxAnthropicRawHeaders    = 128
	maxAnthropicWindows       = 64
	maxAnthropicHeaderValLen  = 1024
	maxAnthropicHeaderNameLen = 256
)
...
func RecordAnthropicRateLimit(auth *cliproxyauth.Auth, headers http.Header, now time.Time) {
	...
	for canonicalName, values := range headers {
		...
		if len(lower) > maxAnthropicHeaderNameLen {
			continue
		}
