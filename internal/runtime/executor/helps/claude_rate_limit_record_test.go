...
func TestRecordAnthropicRateLimit_RetentionBudget(t *testing.T) {
	...
	// An oversized NAME. Capping only the value would let this one header size
	// the capture, both as a raw map key and as the window slug derived from it.
	// Whether it is skipped for its length or for the count budget depends on
	// map iteration order, so the assertion here is the whole-capture bound;
	// TestRecordAnthropicRateLimit_OverLongHeaderNameSkipped isolates the cap.
	headers.Set("Anthropic-Ratelimit-Unified-5h_"+strings.Repeat("a", maxAnthropicHeaderNameLen*3)+"-Status", "allowed")
	...
	for name, value := range hint.RawHeaders {
		...
		if len(name) > maxAnthropicHeaderNameLen {
			t.Errorf("RawHeaders key length = %d, want <= %d", len(name), maxAnthropicHeaderNameLen)
		}
	}
	for slug := range hint.Windows {
		if len(slug) > maxAnthropicHeaderNameLen {
			t.Errorf("window slug length = %d, want <= %d", len(slug), maxAnthropicHeaderNameLen)
		}
	}
}
...
// TestRecordAnthropicRateLimit_OverLongHeaderNameSkipped isolates the name cap.
// It deliberately keeps the capture small: in a capture large enough to exhaust
// the raw-header count budget, an over-long name is dropped by that budget
// instead, so such a test passes whether or not the name is bounded.
func TestRecordAnthropicRateLimit_OverLongHeaderNameSkipped(t *testing.T) {
	authID := "auth-over-long-header-name"
	resetAnthropicHint(t, authID)

	// The slug has to satisfy anthropicWindowSlugPattern, or the header is
	// raw-only and the window-slug bound below never sees it.
	overLongSlug := "5h_" + strings.Repeat("a", maxAnthropicHeaderNameLen*3)
	overLongName := "Anthropic-Ratelimit-Unified-" + overLongSlug + "-Status"

	headers := http.Header{}
	headers.Set("Anthropic-Ratelimit-Unified-5h-Status", "allowed")
	headers.Set(overLongName, "allowed")
	...
	// The ordinary header is still captured: the cap skips one header, it does
	// not abort the capture.
	if _, present := hint.RawHeaders["anthropic-ratelimit-unified-5h-status"]; !present {
		t.Error("ordinary header was dropped, want retained")
	}
	if _, retained := hint.RawHeaders[strings.ToLower(overLongName)]; retained {
		t.Error("over-long header name was retained, want skipped")
	}
	for name := range hint.RawHeaders {
		if len(name) > maxAnthropicHeaderNameLen {
			t.Errorf("RawHeaders key length = %d, want <= %d", len(name), maxAnthropicHeaderNameLen)
		}
	}
	// The slug derives from the name, so an unbounded name is also an unbounded
	// window key — and carryLiveWindows would copy it into later captures.
	for slug := range hint.Windows {
		if len(slug) > maxAnthropicHeaderNameLen {
			t.Errorf("window slug length = %d, want <= %d", len(slug), maxAnthropicHeaderNameLen)
		}
