...
// ClampClaudeRetryAfter caps an already-parsed cooldown at
// maxClaudeRetryAfter and passes nil through unchanged. Every cooldown the
// Claude executor applies goes through this, including the one
// ParseClaudeRateLimitReset produces, because that parser has no upper bound
// of its own: a malformed upstream value (an off-by-1000 ms-vs-s epoch, say)
// would otherwise take a credential out of rotation effectively forever.
func ClampClaudeRetryAfter(d *time.Duration) *time.Duration {
	if d == nil {
		return nil
	}
	return clampedRetryAfter(*d)
}
